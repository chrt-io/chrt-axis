...
export default function generateTicks(ticks, name, callback) {
  ticks.forEach((tick, i, arr) => {
    const dataID = encodeURIComponent(`tick-${name}-${tick.value}`);
    let tickGroup = this.g.querySelector(`[data-id='${dataID}']`);
    if (!tickGroup) {
      tickGroup = create("g");
      ...
      tickLine.setAttribute("stroke-width", this.ticksWidth()(tick, i, arr));
